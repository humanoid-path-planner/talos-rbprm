...
class Robot(Parent):
    ...
    octreeSize = 0.01
    cType = "_6_DOF"
    rLegOffset = [0.0, 0.0, 0.0]
    rLegOffset[2] -= 0.005
    rLegNormal = [0, 0, 1]
    rLegx = 0.1
    rLegy = 0.06

    lLegOffset = [0.0, 0.0, 0.0]
    lLegOffset[2] -= 0.005
    lLegNormal = [0, 0, 1]
    lLegx = 0.1
    lLegy = 0.06

    rArmOffset = [-0.01, 0.0, -0.154]
    rArmNormal = [0, 0, 1]
    rArmx = 0.005
    rArmy = 0.005

    lArmOffset = [-0.01, 0.0, -0.154]
    lArmNormal = [0, 0, 1]
    lArmx = 0.005
    lArmy = 0.005
    ...
    filekin_right = (
        prefix_talos_rbprm()
        / "share/talos-rbprm/com_inequalities/feet_quasi_flat"
        / "COM_constraints_in_RF_effector_frame_REDUCED.obj"
    )
    filekin_left = (
        prefix_talos_rbprm()
        / "share/talos-rbprm/com_inequalities/feet_quasi_flat"
    ...
    file_rf_in_lf = (
        prefix_talos_rbprm()
        / "share/talos-rbprm/relative_effector_positions"
    ...
    kinematic_constraints_path = (
        prefix_talos_rbprm() / "share/talos-rbprm/com_inequalities/feet_quasi_flat/"
    )
    relative_feet_constraints_path = (
        prefix_talos_rbprm() / "share/talos-rbprm/relative_effector_positions/"
    )
    ...
    ref_EE_lArm = np.array(
        [0.13028765672452458, 0.44360498616312666, -0.2881211563246389]
    )
    ref_EE_rArm = np.array(
        [0.13028765672452458, -0.44360498616312666, -0.2881211563246389]
    )
